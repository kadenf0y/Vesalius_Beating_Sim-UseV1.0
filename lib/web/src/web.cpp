...
<style>
/* (Same visual system as current—cleaned & commented) */
...
<body>
  ...
  <div class="main" id="main">
    <div class="leftgrid" id="leftgrid">
      ...
      <div class="strip"><div class="stripHead"><span>Flow</span><span>mL/min</span></div>
      ...
      <div class="strip"><div class="stripHead"><span>Valve Dir</span><span>−1…1</span></div>
        <div class="canvasWrap"><canvas id="cv-valve"></canvas></div>
        <div class="legend"><span class="sw" style="background:#22c55e"></span> -1 (REV) … +1 (FWD)</div>
      </div>
...
function computeBP(){
  ...
  const dir = (valveRaw > 0) ? 1 : 0; // 0 forward-window, 1 reverse-window separation
  if (dir === 0){ if (v > s.sysMax) s.sysMax = v; } else { if (v > s.diaMax) s.diaMax = v; }
  if (s.prevDir === 0 && dir === 1){ s.lastBPText = `${s.sysMax.toFixed(1)}/${s.diaMax.toFixed(1)}`; setText('bp', s.lastBPText); s.sysMax=0; s.diaMax=0; }
  else setText('bp', s.lastBPText);
  s.prevDir = (s.prevDir == null) ? dir : dir;
}

// Controls
let uiBpm=10, uiPowerSet=30, editingBpm=false, editingPow=false, ignoreSseUntil=0;
function clamp(v,lo,hi){ return Math.min(hi,Math.max(lo,v)); }
function stepHold(btn,fn){ let rpt=null; const start=()=>{fn(); rpt=setInterval(fn,140);};
  const stop=()=>{if(rpt){clearInterval(rpt); rpt=null;}}; btn.addEventListener('mousedown',start);
  btn.addEventListener('touchstart',(e)=>{e.preventDefault();start();},{passive:false});
  ['mouseup','mouseleave','touchend','touchcancel'].forEach(ev=>btn.addEventListener(ev,stop));
}
function powerApply(n){ const v=clamp(n|0,10,100); uiPowerSet=v; $('powerLabel').textContent=v+'%'; fetch('/api/power?pct='+v).catch(()=>{}); }
function bpmApply(n){ const v=clamp(n|0,1,60); uiBpm=v; $('bpmLabel').textContent=String(v); fetch('/api/bpm?b='+v).catch(()=>{}); }
...
(function bind(){
  ...
  const pField=$('powerLabel'); pField.addEventListener('focus',()=>{editingPow=true;});
  pField.addEventListener('keydown',(e)=>{if(e.key==='Enter'){e.preventDefault();pField.blur();}});
  pField.addEventListener('blur',()=>{ const n=parseInt(String(pField.textContent).replace(/[^\d\-]/g,''),10);
    if (isNaN(n)) { pField.textContent = uiPowerSet + '%'; editingPow=false; return; }
  ...
  const bField=$('bpmLabel'); bField.addEventListener('focus',()=>{editingBpm=true;});
  bField.addEventListener('keydown',(e)=>{if(e.key==='Enter'){e.preventDefault();bField.blur();}});
  bField.addEventListener('blur',()=>{ const n=parseInt(String(bField.textContent).replace(/[^\d\-]/g,''),10);
    if (isNaN(n)) { bField.textContent = String(uiBpm); editingBpm=false; return; }
    editingBpm=false; bpmApply(n);
  });
  function setModeButtons(m){ document.querySelectorAll('#modeSeg button').forEach(b=> b.classList.toggle('active', Number(b.dataset.m)===m)); }
  document.querySelectorAll('#modeSeg button').forEach(btn=> btn.addEventListener('click',()=>{ const m=Number(btn.dataset.m)||0; setModeButtons(m); fetch('/api/mode?m='+m).catch(()=>{}); }));
  $('btnPause').onclick=()=>{ const btn=$('btnPause'); const showingPlay=btn.classList.contains('go'); btn.disabled=true;
    if (showingPlay){ btn.classList.remove('go'); btn.classList.add('stop'); btn.textContent='Pause'; } else { btn.classList.remove('stop'); btn.classList.add('go'); btn.textContent='Play'; }
    ignoreSseUntil = performance.now()+400; fetch('/api/toggle').finally(()=> setTimeout(()=>btn.disabled=false,250));
  };
...
function valveToSigned(v, fallbackMode){ if (typeof v==='number'){ if (v===-1||v===0||v===1) return v; if (v>0) return 1; return -1; }
  if (v===true) return 1; if (v===false) return -1; return (fallbackMode===1)?-1:1; }
es.onmessage=(ev)=>{
  ...
  try{
    ...
    const signedValve = valveToSigned(d.valve, Number(d.mode)); stripValve.push(signedValve);
    const pwmRaw = (typeof d.pwm === 'number') ? d.pwm : PWM_FLOOR; stripPower.push(pwmRaw);
    $('n-atr').textContent  = (Number(d.atr_mmHg)||0).toFixed(1);
    $('n-vent').textContent = (Number(d.vent_mmHg)||0).toFixed(1);
    $('n-flow').textContent = (Number(d.flow_ml_min)||0).toFixed(1);
    $('n-valve-arrow').textContent = (signedValve>0)?'▲':(signedValve<0)?'▼':'-';
    $('n-power').textContent = String(pwmRaw);
    if (typeof d.powerPct === 'number' && !document.activeElement || document.activeElement.id!=='powerLabel'){ $('powerLabel').textContent = Math.round(d.powerPct)+'%'; }
    if (typeof d.bpm === 'number' && !document.activeElement || document.activeElement.id!=='bpmLabel'){ $('bpmLabel').textContent = String(Math.round(Number(d.bpm)||0)); }
    ...
    if (performance.now() > ignoreSseUntil){ const paused = !!d.paused; const btn=$('btnPause');
      if (paused){ btn.classList.remove('stop'); btn.classList.add('go'); btn.textContent='Play'; } else { btn.classList.remove('go'); btn.classList.add('stop'); btn.textContent='Pause'; }
    }
    computeBP();
  }catch(e){}
};
...
static void sse_task(void*) {
  ...
  for(;;){
    ...
    const float  vent = scale_vent(G.vent_mmHg.load(std::memory_order_relaxed)); // NOTE: control wrote pre-cal; we apply UI-cal
